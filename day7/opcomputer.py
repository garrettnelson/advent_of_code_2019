...
class Amplifiers(object):
	def __init__(self,file_path,amps,loop=None):
		self.amps = amps
	...
	def fire_amps(self):
		...
		if self.loop == None:
			for amp in self.amps:
				o = OPcodeComputer(self.file_path,amp,last_output)
				last_output = o.process_instructions()
			return last_output
		else:
			for i in range(len(amp)):
				self.amp_comps.append(OPcodeComputer(self.file_path,amp,last_output,to))
			while True:
				amp = self.amps[current_amp]
				to = None
				if not self.test_override == None:
					to = self.test_override.split(',')
				o = OPcodeComputer(self.file_path,amp,last_output,to)
				signal = o.process_instructions()
				if signal == '99':
					break
				last_output = signal
				if current_amp == len(self.amps) -1:
					current_amp = -1
				current_amp += 1
			return last_output
...
class OPcodeComputer(object):
	def __init__(self,input_path=None,parm=None,amp=None,debug=False,op_debug=False,param_debug=False):
		...
		try:
			...
			self.instructions = input_path.split(",")
		self.current_position = 0
	def process_instructions(self,parm=None):
		if not parm == None:
			self.parm = parm
			self.consumed_input = False 
	...
	def process_instruction(self,position):
		...
		if result != None:
			if result == -1:
				if self.get_op_code() == "04":
					self.current_position += self.get_next_op_position()
				else:
	...
	def op_03(self,position):
		value = self.amp if self.consumed_input else self.parm
		if self.debug == True:
			print "Setting Position %s to %s" % (self.get_result_position(),value)
		self.instructions[self.get_result_position()] = "%s" % (value)
