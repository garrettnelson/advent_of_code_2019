...
def move_wire(x,y,direction,wire_name,steps):
	if direction == 'R':
		...
		y = y-1
	does_point_exist= (x,y) in board
	if does_point_exist:
		existing_point = board[x,y]
		if existing_point.has(wire_name):
			return False,existing_point
		else:
			existing_point.add_wire(wire_name,steps)
			return False,existing_point
	else:
		p = Point(x,y,wire_name,steps)
		board[x,y] = p
		return False,p

def draw_wire(input,wire_name):
	x,y = 0,0
	...
	for move in movements:
		...
		for i in range(length):
			steps += 1
			is_new_point,point = move_wire(x,y,direction,wire_name,steps)
			x = point.x
			y = point.y
			if is_new_point:
				steps = point.steps_for(wire_name)
draw_wire(wire1,"wire1")
draw_wire(wire2,"wire2")
#find intersections
print "Total moves: %s" % (total_length(wire1)+total_length(wire2))
print len(board.keys())
intersections = []
for x,y in board.keys():
	if board[x,y].is_intersection():
		intersections.append(board[x,y])
min_e =  min(intersections,key=Point.e_distance)
print min_e.e_distance()
min_steps = min(intersections,key=Point.wire_distance)
print min_steps.wire_distance()